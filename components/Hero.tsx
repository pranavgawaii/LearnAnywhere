...
const Hero: React.FC<HeroProps> = ({ onJoinClick }) => {
  return (
    <section className="relative w-full overflow-hidden bg-white dark:bg-slate-950 pt-32 pb-64 lg:pt-48 lg:pb-80 transition-colors duration-300 border-b border-slate-100 dark:border-slate-800">

      {/* Professional Dynamic Background */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        {/* Central Glow */}
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-[1000px] bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-brand-teal/5 via-brand-primary/5 to-transparent opacity-60"></div>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10 flex flex-col items-center text-center">
        ...
        <motion.div
          ...
          className="inline-flex items-center gap-2 pl-1 pr-4 py-1.5 rounded-full bg-slate-50/80 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 backdrop-blur-md shadow-sm mb-8 hover:shadow-md hover:border-brand-teal/30 transition-all duration-300 cursor-default"
        >
          ...
          <span className="text-sm font-medium text-slate-600 dark:text-slate-300 flex items-center gap-1">
            Lumina 2.0 is live <ArrowRight size={14} className="text-slate-400" />
          </span>
        </motion.div>
        ...
        <motion.h1
          ...
          className="max-w-4xl mx-auto text-5xl sm:text-6xl lg:text-8xl font-bold tracking-tight text-slate-900 dark:text-white leading-[1.05] mb-8 text-center"
        >
        ...
        {/* Subheadline - Explicitly Centered */}
        <motion.p
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6, delay: 0.2 }}
          className="text-xl md:text-2xl text-slate-600 dark:text-slate-400 max-w-2xl mx-auto leading-relaxed mb-12 text-center"
        >
        ...
        {/* Social Proof / Stats - Explicitly Centered */}
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ duration: 0.8, delay: 0.5 }}
          className="flex flex-wrap items-center justify-center gap-x-12 gap-y-6 pt-12 border-t border-slate-100 dark:border-slate-800/50 max-w-4xl mx-auto w-full"
        >
